        ...
        <div className="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0">
          <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <form onSubmit={handleSubmit(props.addNewProduct)} className="flex flex-col">
                <h1 className="text-2xl font-bold text-blue-700 pb-5">Add Product</h1>
                <div className="flex flex-col">
                  ...
                  <select className="my-3 border-2 p-2 rounded-md" required>
                    {categories.map((category) => (
                      <option key={category.id} value={category.id} {...register('categoryId')}>
                        {category.name}
                      </option>
                    ))}
                  </select>
